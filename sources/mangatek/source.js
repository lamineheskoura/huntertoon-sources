function createSource(api, config) {
  ...
  function extractHref(item, fallbackSelector) {
  ...
  function extractImageFromAttrs(attrs) {
    var url = (attrs && (attrs["data-src"] || attrs["data-lazy-src"] || attrs.src || attrs["data-original"])) || "";
  ...
  async function toMangaList(html, listSel, opts) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      var href = extractHref(item, opts.urlSel || "a");
      ...
      var cover = await api.cssAttr(itemHtml, imgSel, "data-src");
      if (!cover) cover = await api.cssAttr(itemHtml, imgSel, "data-lazy-src");
      if (!cover) cover = await api.cssAttr(itemHtml, imgSel, "src");
      if (!cover && item.attrs) cover = item.attrs.src || item.attrs["data-src"] || "";
      cover = processImageUrl(cover);

      var title = await api.cssText(itemHtml, opts.titleSel || "h3, .manga-title, .tt");
  ...
  return {
    ...
    async getHomepageManga(args) {
      try {
        ...
        var html = await fetchHtml(page === 1 ? baseUrl + "/latest" : baseUrl + "/latest?page=" + page);
        return await toMangaList(html, ".chapter-card", {
          titleSel: "h4.chapter-title, h3, .manga-title, .tt",
          coverSel: "img",
          urlSel: "a[href^='/manga/']"
        });
      } catch (e) { return []; }
    ...
    async search(args) {
      try {
        ...
        return await toMangaList(html, sel("search_list", "a[href^='/manga/'], .manga-card, .listupd .bs"), {
          titleSel: sel("search_title", "h3, .manga-title, .tt"),
    ...
    async getMangaDetails(args) {
      ...
      var cover = (mangaData && mangaData.cover_image) || await api.cssAttr(html, sel("manga_cover", ".manga-cover img, .thumb img"), "data-src") || await api.cssAttr(html, sel("manga_cover", ".manga-cover img, .thumb img"), "src") || await api.cssAttr(html, "meta[property='og:image']", "content") || "";
